function showPlot2(X,wf,Y_rgb,d,PSF,j,showMTF,ploton,algo)
...
wf = mat2gray(double(wf)).*1;
X = mat2gray(double(X)).*1;
for k=1:K
...
if(ploton==1)
    rgb = RGB_test(X);
    subplot(3,K+1,1),imagesc(rgb(:,:,[1 2 3])),title("Groundtruth Image "+num2str(j)),pbaspect([1 1 1]),axis off;
end
if(ploton==1)
    for k=1:K
        rgb = RGB_test(Y_rgb(:,:,:,k));
        subplot(3,K+1,k+1),imagesc(rgb(:,:,[1 2 3])),title("Measurement z="+ num2str(z(k))+"[m]"),axis off,pbaspect([1 1 1])
    end
...
for k=1:K
    ...
    % s1 = si(k);
    if(ploton==1)
        subplot(3,K+1,K+2),plot(linspace(-Ns,Ns,N),d(Ns,:)),title("DOE profile"),axis([-Ns Ns 0 max(d(Ns,:))+eps]);
        if(showMTF == 1)
            for l=1:L
                H(:,:,l)  = psf2otf(PSF(:,:,l,k));
            ...
            rgb = RGB_test(h1);
            subplot(3,K+1,(K+1)+k+1),imagesc((rgb(:,:,[1 2 3]).^0.25)),title("PSF "+"z="+ num2str(z(k))+"[m]"),pbaspect([1 1 1]),axis off;
            %pause(0.1);
            colormap('jet')
        end
        if(ploton==1)
            rgb = RGB_test(wftemp1);
            subplot(3,K+1,2*(K+1)+k+1),imagesc(rgb(:,:,[1 2 3])),title(algorithm(algo)+" z="+ num2str(z(k))+"[m]" +", PSNR="+ num2str(p(k))),pbaspect([1 1 1]),axis off;
            colormap('jet')
        end
    end
end
